...
def parse_args():
    parser = argparse.ArgumentParser(description="ACS bronze to silver transformation")
    parser.add_argument(
        "--input-csv",
        default="gs://mgmt405_dataset/bronze/acs_datasets/ACS_ZCTA_PHILADELPHIA_NEW_ORLEANS_2021.csv",
        help="Input ACS CSV path (local path or gs:// path)",
    )
    parser.add_argument(
        "--output-path",
        default="gs://mgmt405_dataset/silver/acs/ACS_ZCTA_PHILADELPHIA_NEW_ORLEANS_2021_WITH_CUSTOM_COLUMNS",
        help="Output parquet path (local path or gs:// path)",
    )
    parser.add_argument(
        "--write-mode",
        default="overwrite",
        help="Spark write mode (overwrite, append, ignore, errorifexists)",
    )
...
def main():
    ...
    spark = SparkSession.builder.appName("ACS_ETL_Pipeline").getOrCreate()
    ...
    # -----------------------------
    df_filtered = (
        df_filtered.withColumn("pct_kids", F.col("age_0_17") / F.col("total_population"))
    ...
    df_filtered = df_filtered.withColumn(
        "upper_middle_hh",
        F.col("hh_75k_100k") + F.col("hh_100k_125k") + F.col("hh_125k_150k"),
    )
    ...
    # -----------------------------
    df_filtered = (
        df_filtered.withColumn("pct_low_income", F.col("low_income_hh") / F.col("total_households"))
    ...
    df_filtered.write.mode(args.write_mode).format("parquet").save(output_path)
